...
func (w *WAL) Append(key string, value []byte, tombstone bool) error {
	...
	log := Serialize(key, value, tombstone)
...
func Serialize(key string, value []byte, tombstone bool) []byte {
	log := make([]byte, 0)
...
func (w *WAL) Replay(fn func(key string, value []byte, tombstone bool)) error {
	...
	for {
		key, val, tombstone, err := ReadLog(w.f)
		if err != nil {
...
// Helper method to read a chunk of log from disk
//
// it returns the key, value and tombstone
func ReadLog(f io.Reader) (string, []byte, bool, error) {
	totalLength := make([]byte, 4)

	r, err := io.ReadFull(f, totalLength)
	if err == io.EOF && r == 0 {
		return "", nil, false, err // clean EOF — no more records
	}
	if errors.Is(err, io.ErrUnexpectedEOF) {
		return "", nil, false, ErrTruncatedFile // partial length prefix
	}
	if err != nil {
		return "", nil, false, fmt.Errorf("%w: %v", ErrWALRead, err)
	}
	// read the total chunk of a log
	log := make([]byte, binary.LittleEndian.Uint32(totalLength))
	r, err = io.ReadFull(f, log)
	if errors.Is(err, io.ErrUnexpectedEOF) || errors.Is(err, io.EOF) {
		return "", nil, false, ErrTruncatedFile
	}
	if err != nil {
		return "", nil, false, fmt.Errorf("%w: %v", ErrWALRead, err)
	}
	...
	if crc != crcCheck {
		return "", nil, false, ErrWALCRCCheckFail
	}
	// read the tombstone
